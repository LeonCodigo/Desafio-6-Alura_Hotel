...
//Activa el evento, después del usuario seleccionar las fechas se debe calcular el valor de la reserva
                  ...
                  {
                	java.util.Date entrada = (java.util.Date) txtFechaE.getDate();
                  	java.util.Date salida = (java.util.Date) txtFechaS.getDate();
        				
        		   int estadia =  diferencia_dias(entrada, salida);
        		   int valorEstadia = estadia * tarifa;
        		   String valorEstadiaString= Integer.toString(valorEstadia);
        		   txtValor.setText(valorEstadiaString);
		...
		txtValor.setBounds(148, 328, 132, 33);
		...
		lblValor.setBounds(72, 303, 229, 14);
		...
		btnsiguiente.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				{		
					...
					Exito cartelExito = new Exito();
					JOptionPane.showMessageDialog(btnsiguiente, "Registrado con exito");
				} 
				else
				{
	...
	/*
	 * public static JDateChooser txtFechaE;         -->> Elemento grafico de biblioteca JCalendar, muestra opciones y recibe los datos de la fecha de entrada;        
	   public static JDateChooser txtFechaS;         -->> recibe los datos de la fecha de Salida;   
	   public static JComboBox<Format> txtFormaPago; -->> Elemento grafico de biblioteca JCalendar, muestra opciones en una lista desplegable de formas de pago
	 * */
	
	public void guardarReserva()
	{
		String fechaEn = ((JTextField) txtFechaE.getDateEditor().getUiComponent()).getText();
		String fechaSal =  ((JTextField) txtFechaS.getDateEditor().getUiComponent()).getText();
		Reserva reserva = new Reserva(java.sql.Date.valueOf(fechaEn), java.sql.Date.valueOf(fechaSal), txtValor.getText(),txtFormaPago.getSelectedItem().toString() );
	    reservaController.guardar(reserva);
	}
	
	
	
	public Integer diferencia_dias(java.util.Date pEntrada, java.util.Date pSalida) 
	{
		
		LocalDate entrada = LocalDate.of(pEntrada.getYear(),pEntrada.getMonth(),pEntrada.getDate()) ;
		LocalDate salida = LocalDate.of(pSalida.getYear(),pSalida.getMonth(),pSalida.getDate()) ;
		Integer res=0;
		long resDate = entrada.until(salida, ChronoUnit.DAYS);
	...
	/*
	  JDateChooser                      -->> Elemento grafico de biblioteca JCalendar, muestra opciones y recibe los datos;
	  JTextField                        -->> Elemento de texto, Lo usamos como cast para pasarle el dato a la variable final String 
	  getDateEditor().getUiComponent()  -->> Metodos para obtener la informacion desde la interfaz grafica.
	  getText()                         -->> Obtenemos el dato en formato texto. que va a ser casteado con (JTextField).
	  
	  java.sql.Date.valueOf(fechaEn)    -->> Reserva espera un tipo SQL Date, se pasa de String a ese formato. 
	*/
